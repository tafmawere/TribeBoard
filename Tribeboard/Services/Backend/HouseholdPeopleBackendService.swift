...
protocol HouseholdPeopleBackendService {
    func fetchPeople(householdId: UUID, session: AuthUserSession) async throws -> [BackendHouseholdPerson]
    func createPerson(_ person: BackendHouseholdPerson, session: AuthUserSession) async throws -> BackendHouseholdPerson
    func updatePerson(_ person: BackendHouseholdPerson, session: AuthUserSession) async throws -> BackendHouseholdPerson
    func deletePerson(id: UUID, session: AuthUserSession) async throws
}

struct SupabaseHouseholdPeopleBackendService: HouseholdPeopleBackendService {
    ...
    func fetchPeople(householdId: UUID, session: AuthUserSession) async throws -> [BackendHouseholdPerson] {
        let path = "household_people?select=*&household_id=eq.\(householdId.uuidString)&order=created_at.asc"
        return try await get(
            path: path,
            session: session,
            readContext: ReadDiagnosticContext(operation: "fetchPeople", householdId: householdId)
        )
    }

    func createPerson(_ person: BackendHouseholdPerson, session: AuthUserSession) async throws -> BackendHouseholdPerson {
        let created: [BackendHouseholdPerson] = try await post(
            table: "household_people",
            payload: [person],
            session: session,
            context: WriteDiagnosticContext(
                operation: "createHouseholdPerson",
                tableOrEndpoint: "household_people",
                payloadSummary: "id=\(person.id.uuidString), household_id=\(person.householdId.uuidString), role=\(person.role)"
            )
        )
        guard let first = created.first else {
            throw ServiceError.requestFailed("Failed to create household person.")
        }
        return first
    }

    func updatePerson(_ person: BackendHouseholdPerson, session: AuthUserSession) async throws -> BackendHouseholdPerson {
        let path = "household_people?id=eq.\(person.id.uuidString)"
        let updated: [BackendHouseholdPerson] = try await patch(
            path: path,
            payload: [person],
            session: session,
            context: WriteDiagnosticContext(
                operation: "updateHouseholdPerson",
                tableOrEndpoint: "household_people",
                payloadSummary: "id=\(person.id.uuidString), household_id=\(person.householdId.uuidString), role=\(person.role)"
            )
        )
        guard let first = updated.first else {
            throw ServiceError.requestFailed("Failed to update household person.")
        }
        return first
    }

    func deletePerson(id: UUID, session: AuthUserSession) async throws {
        let path = "household_people?id=eq.\(id.uuidString)"
        _ = try await delete(
            path: path,
            session: session,
            context: WriteDiagnosticContext(
                operation: "deleteHouseholdPerson",
                tableOrEndpoint: "household_people",
                payloadSummary: "id=\(id.uuidString)"
            )
        ) as [BackendHouseholdPerson]
    }
    ...
    private func get<T: Decodable>(
        ...
        request.allHTTPHeaderFields = try authenticatedHeaders(
            session: session,
            request: request,
            operation: "fetchHouseholdPeople",
    ...
    private func perform<T: Decodable>(
        ...
        if let readContext {
            print(
                "[HouseholdPeopleBackendService] \(readContext.operation) " +
...
#if DEBUG
            print(
                "[HouseholdPeopleBackendService] DECODE FAILED type=\(String(describing: T.self)), " +
